...
function NewsTicker() {
  ...
  const renderHeadlines = (repeatIndex: number, ariaHidden: boolean) =>
    items.map((item, i) => (
      <span key={`${repeatIndex}-${i}`} className="inline-flex items-center">
        {item.id ? (
          <button
            type="button"
  ...
  return (
    <div className="flex items-stretch overflow-hidden border-t border-gold/30 bg-navy-deep text-primary-foreground">
      ...
      <div className="relative flex-1 overflow-hidden py-1.5">
        <div
          key={items.map((it) => it.title).join("|")}
          className="md-ticker-track w-max px-4 text-sm"
          style={{ animationPlayState: paused ? "paused" : "running" }}
        >
          {renderHeadlines(0, false)}
          <span className="px-4" aria-hidden="true">•</span>
          {renderHeadlines(1, true)}
          <span className="px-4" aria-hidden="true">•</span>
          {renderHeadlines(2, true)}
        </div>
      </div>
    </div>
  );
