...
import { Card, ThumbArt, SectionTitle, Pill, Button, cx } from "./ui";
...
function AllArticlesDropdown({ articles }: { articles: Article[] }) {
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      {open && (
        <div className="absolute top-full z-30 mt-2 w-72 max-w-[85vw] overflow-hidden rounded-lg border border-border bg-background shadow-lg end-0">
...
const SEARCH_CATEGORIES = ["عملات رقمية", "سياسة", "اقتصاد", "مجتمع"];

function SearchBox({ articles }: { articles: Article[] }) {
  const { navigate } = useMadar();
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      {open && (
        <div className="absolute top-full z-30 mt-2 w-80 max-w-[88vw] overflow-hidden rounded-lg border border-border bg-background shadow-lg start-0">
          <div className="border-b border-border p-2">
            <input
              autoFocus
              type="text"
            ...
            {filtered.length > 0 ? (
              filtered.map((a) => (
                <button
                  ...
                  className="block w-full border-b border-border/60 px-3 py-2 text-start text-xs leading-snug hover:bg-secondary last:border-b-0"
                >
                  <span className="block font-bold text-navy">{a.title}</span>
